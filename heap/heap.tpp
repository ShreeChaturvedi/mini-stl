...
T& TEMPLATED_HEAP::emplace(Args&&... args) {
...
const T TEMPLATED_HEAP::pop() {
    if (empty()) throw std::out_of_range("Cannot pop from empty heap");
...
void TEMPLATED_HEAP::heapify(std::size_t pos) {
    ...
    for (; child < std::min(end, data_.size() + 1); ++child) {
        if (compare{}(at(child), at(extremum))) extremum = child;
    }
    if (extremum == pos) return;
...
void TEMPLATED_HEAP::build_heap() {
    for (std::size_t pos = data_.size(); pos > 0; --pos) heapify(pos);
}
