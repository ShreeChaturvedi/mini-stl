...
typename Vector<T>::iterator Vector<T>::emplace(const_iterator pos, Args&&... args) {
  ...
  if (index > size_) throw std::out_of_range("Vector::emplace position out of range");
  ...
  if constexpr (std::is_move_assignable_v<T> || std::is_copy_assignable_v<T>) {
    ...
    if constexpr (std::is_nothrow_move_assignable_v<T> || !std::is_copy_assignable_v<T>) {
      for (std::size_t i = old_size - 1; i > index; --i) data_[i] = std::move(data_[i - 1]);
    } else {
      for (std::size_t i = old_size - 1; i > index; --i) data_[i] = data_[i - 1];
    }
...
typename Vector<T>::iterator Vector<T>::erase(const_iterator pos) {
  const std::size_t index = static_cast<std::size_t>(pos - cbegin());
  if (index >= size_) throw std::out_of_range("Vector::erase position out of range");
...
typename Vector<T>::iterator Vector<T>::erase(const_iterator first, const_iterator last) {
  ...
  if (first_index > last_index) throw std::out_of_range("Vector::erase invalid range");
  if (last_index > size_) throw std::out_of_range("Vector::erase range out of range");
    ...
    try {
      ...
      if constexpr (std::is_nothrow_move_constructible_v<T>) {
        std::uninitialized_move_n(
            data_ + last_index, size_ - last_index, new_data + first_index);
      } else {
