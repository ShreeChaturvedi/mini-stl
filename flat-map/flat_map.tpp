...
typename FlatMap<Key, T, Compare>::iterator FlatMap<Key, T, Compare>::lower_bound(
    const Key& key) noexcept {
  return std::lower_bound(
      data_.begin(), data_.end(), key, [this](const value_type& v, const Key& k) {
        return comp_(v.first, k);
      });
}

template <typename Key, typename T, typename Compare>
typename FlatMap<Key, T, Compare>::const_iterator FlatMap<Key, T, Compare>::lower_bound(
...
std::pair<typename FlatMap<Key, T, Compare>::iterator, bool> FlatMap<Key, T, Compare>::emplace(
    Args&&... args) {
...
template <typename Key, typename T, typename Compare>
typename FlatMap<Key, T, Compare>::iterator FlatMap<Key, T, Compare>::find(
    const Key& key) noexcept {
  auto it = lower_bound(key);
  if (it != end() && keys_equal(comp_, it->first, key)) return it;
  return end();
}

template <typename Key, typename T, typename Compare>
typename FlatMap<Key, T, Compare>::const_iterator FlatMap<Key, T, Compare>::find(
    const Key& key) const noexcept {
  auto it = lower_bound(key);
  if (it != end() && keys_equal(comp_, it->first, key)) return it;
  return end();
