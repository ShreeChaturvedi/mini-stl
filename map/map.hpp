...
private:
  struct key_of_value {
    const K& operator()(const value_type& v) const noexcept { return v.first; }
  };
...
public:
  ...
  V& operator[](const K& key) {
    auto [it, inserted] = insert(value_type(key, V{}));
    (void)inserted;
  ...
  V& at(const K& key) {
    auto it = find(key);
    if (it == end()) throw std::out_of_range("map::at missing key");
    return const_cast<V&>(it->second);
  }
...
public:
  ...
  multimap() = default;
  explicit multimap(Compare comp) : tree_(std::move(comp)) {}
