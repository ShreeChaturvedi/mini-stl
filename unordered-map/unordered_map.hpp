...
public:
  ...
  void max_load_factor(float f) noexcept { max_load_factor_ = (f <= 0.0f) ? 1.0f : f; }
  ...
  void reserve(size_type n) {
    ...
    if (needed > bucket_count()) rehash(needed);
  }
...
private:
  ...
  size_type bucket_of(const K& key) const { return Hash{}(key) % bucket_count(); }

  void maybe_rehash_for_insert() {
    const float projected = static_cast<float>(size_ + 1) / static_cast<float>(bucket_count());
    if (projected > max_load_factor_) rehash(bucket_count() * 2);
  }
...
V& unordered_map<K, V, Hash, KeyEqual>::at(const K& key) {
  ...
  auto& list = buckets_[bucket];
  for (auto& kv : list) {
    if (KeyEqual{}(kv.first, key)) return kv.second;
  }
...
public:
  ...
  static base_iterator begin(std::conditional_t<std::is_const_v<T_value>, const unordered_map*, unordered_map*> map) {
    base_iterator it(map, 0, {});
    it.skip_empty_buckets();
    return it;
  }

  static base_iterator end(std::conditional_t<std::is_const_v<T_value>, const unordered_map*, unordered_map*> map) {
    return base_iterator(map, map->bucket_count(), {});
  }

  base_iterator& operator++() {
    if (bucket_ == map_->bucket_count()) return *this;
...
private:
  using map_ptr = std::conditional_t<std::is_const_v<T_value>, const unordered_map*, unordered_map*>;
  ...
  void skip_empty_buckets() {
    while (bucket_ < map_->bucket_count() && map_->buckets_[bucket_].empty()) ++bucket_;
