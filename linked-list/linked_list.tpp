...
std::size_t LinkedList<T>::insert(T element, std::size_t index) {
  if (index > size_) throw std::invalid_argument("Supplied index out of bounds");

  Node* prev = node_before(index);
  Node* next = prev->next;
  prev->next = new Node(element, next);
  if (next == nullptr) end_ = prev->next;
  return ++size_;
}
