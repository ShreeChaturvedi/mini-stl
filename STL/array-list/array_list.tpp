...
ArrayList<T>::ArrayList(std::size_t length, T value)
    ...
    for (std::size_t pos = 0; pos < length; ++pos) data_[pos] = value;
}

template <typename T>
ArrayList<T>::ArrayList(const ArrayList& other) : size_(other.size_),
...
template <typename T>
ArrayList<T>::ArrayList(ArrayList&& other) : size_(other.size_), 
    capacity_(other.capacity_) {
    data_ = other.data_;
...
template <typename T>
std::size_t ArrayList<T>::capacity() const noexcept {
    return capacity_;
}
