...
SmallVector<T, InlineCapacity>::SmallVector() noexcept
    : size_(0), capacity_(inline_capacity()), data_(inline_capacity() == 0 ? nullptr : inline_data()) {}
...
template <typename T, std::size_t InlineCapacity>
SmallVector<T, InlineCapacity>::SmallVector(SmallVector&& other) noexcept(std::is_nothrow_move_constructible_v<T>)
    : SmallVector() {
  if (!other.using_inline_storage()) {
    data_ = std::exchange(other.data_, other.inline_capacity() == 0 ? nullptr : other.inline_data());
...
template <typename T, std::size_t InlineCapacity>
SmallVector<T, InlineCapacity>& SmallVector<T, InlineCapacity>::operator=(const SmallVector& other) {
...
template <typename T, std::size_t InlineCapacity>
SmallVector<T, InlineCapacity>& SmallVector<T, InlineCapacity>::operator=(SmallVector&& other) noexcept(
    std::is_nothrow_move_constructible_v<T>) {
  if (this == &other) return *this;

  clear();
  if (!using_inline_storage()) deallocate(data_, capacity_);
...
template <typename T, std::size_t InlineCapacity>
SmallVector<T, InlineCapacity>::~SmallVector() {
  clear();
  if (!using_inline_storage()) deallocate(data_, capacity_);
}
...
T& SmallVector<T, InlineCapacity>::at(size_type i) {
  if (i >= size_) throw std::out_of_range("SmallVector::at out of range");
  return data_[i];
}
...
template <typename T, std::size_t InlineCapacity>
T& SmallVector<T, InlineCapacity>::front() {
  if (empty()) throw std::out_of_range("SmallVector::front on empty");
  return data_[0];
}
...
template <typename T, std::size_t InlineCapacity>
T& SmallVector<T, InlineCapacity>::back() {
  if (empty()) throw std::out_of_range("SmallVector::back on empty");
...
void SmallVector<T, InlineCapacity>::pop_back() {
  if (empty()) throw std::out_of_range("SmallVector::pop_back on empty");
...
template <typename T, std::size_t InlineCapacity>
template <typename... Args>
typename SmallVector<T, InlineCapacity>::iterator SmallVector<T, InlineCapacity>::emplace(
    const_iterator pos,
    Args&&... args) {
  const size_type index = static_cast<size_type>(pos - cbegin());
  if (index > size_) throw std::out_of_range("SmallVector::emplace position out of range");
  ...
  if constexpr (std::is_move_assignable_v<T> || std::is_copy_assignable_v<T>) {
    ...
    if constexpr (std::is_nothrow_move_assignable_v<T> || !std::is_copy_assignable_v<T>) {
      for (size_type i = old_size - 1; i > index; --i) data_[i] = std::move(data_[i - 1]);
    } else {
      for (size_type i = old_size - 1; i > index; --i) data_[i] = data_[i - 1];
    }
...
template <typename T, std::size_t InlineCapacity>
typename SmallVector<T, InlineCapacity>::iterator SmallVector<T, InlineCapacity>::erase(const_iterator pos) {
  const size_type index = static_cast<size_type>(pos - cbegin());
  if (index >= size_) throw std::out_of_range("SmallVector::erase position out of range");
  return erase(cbegin() + index, cbegin() + index + 1);
}

template <typename T, std::size_t InlineCapacity>
typename SmallVector<T, InlineCapacity>::iterator SmallVector<T, InlineCapacity>::erase(
  ...
  if (first_index > last_index) throw std::out_of_range("SmallVector::erase invalid range");
  if (last_index > size_) throw std::out_of_range("SmallVector::erase range out of range");
...
template <typename T, std::size_t InlineCapacity>
void SmallVector<T, InlineCapacity>::swap(SmallVector& other) noexcept(std::is_nothrow_move_constructible_v<T>) {
