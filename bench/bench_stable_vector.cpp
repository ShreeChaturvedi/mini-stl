...
BENCH_CASE("stable_vector/push_back") {
  ...
  for (std::size_t i = 0; i < n; ++i) values[i] = dist(rng);

  stl_bench::run_samples("StableVector<int>::push_back", n, [&] {
    StableVector<int> xs;
    xs.reserve(n);
    for (std::size_t i = 0; i < n; ++i) xs.push_back(values[i]);
  ...
  stl_bench::run_samples("std::vector<unique_ptr<int>>::push_back", n, [&] {
    ...
    xs.reserve(n);
    for (std::size_t i = 0; i < n; ++i) xs.push_back(std::make_unique<int>(values[i]));
    stl_bench::do_not_optimize(xs.size());
  });
}
